...
foreach my $prog (@optional_dependent_programs) {
	...
	chomp $which_output;
	if ($which_output !~ /^\/.*\/$prog$/) {
		print "WARNING: $prog is not in your path.  Check that package is installed and your 
PATH is sane.\n";
		$odp_present{$prog} = 0;
	} else {
		print "[V] Dependent program \"$prog\" found in $which_output\n" if $verbose;
...
print "Starting enum4linux v$VERSION ( http://labs.portcullis.co.uk/application/enum4linux/ ) on " .  
scalar(localtime) . "\n";
...
sub get_domain_sid {
	...
	my $command = "rpcclient -W '$global_workgroup' -U'$global_username'\%'$global_password' 
$global_target -c 'lsaquery' 2>&1";
...
$verbose;
	...
	unless ($global_workgroup) {
		...
		unless (defined($global_workgroup) and $global_workgroup =~ /^[A-Za-z0-9_\.-]+$/) {
			print "ERROR: Workgroup \"$global_workgroup\"contains some illegal 
characters\n";
			exit 1;
		}
	}
...
sub get_ldapinfo {
	...
	unless ($odp_present{"ldapsearch"}) {
		print "[E] Dependent program \"ldapsearch\" not present.  Skipping this check.  
Install ldapsearch to fix.\n\n";
		return 0;
	}
	...
	foreach my $line (split "\n", $output) {
		if ($line =~ /namingContexts: DC=DomainDnsZones/ or $line =~ /namingContexts: 
DC=ForestDnsZones/) {
			$parent = 1;
...
sub make_session {
	...
	my $command = "smbclient -W '$global_workgroup' //'$global_target'/ipc\$ 
-U'$global_username'\%'$global_password' -c 'help' 2>&1";
	print "[V] Attempting to make null session using command: $command\n" if $verbose;
	my $os_info = `$command`;
	chomp $os_info;
	if ($os_info =~ /case_sensitive/) {
		print "[+] Server $global_target allows sessions using username '$global_username', 
password '$global_password'\n";
	} else {
		print "[E] Server doesn't allow session using username '$global_username', password 
'$global_password'.  Aborting remainder of tests.\n";
		exit 1;
	}
...
sub get_os_info {
	...
	my $command = "smbclient -W '$global_workgroup' //'$global_target'/ipc\$ 
-U'$global_username'\%'$global_password' -c 'q' 2>&1";
	...
	$command = "rpcclient -W '$global_workgroup' -U'$global_username'\%'$global_password' -c 
'srvinfo' '$global_target' 2>&1";
	print "[V] Attempting to get OS info with command: $command\n" if $verbose;
	$os_info = `$command`;
	if (defined($os_info)) {
...
sub enum_password_policy {
	...
	unless ($odp_present{"polenum.py"}) {
		print "[E] Dependent program \"polenum.py\" not present.  Skipping this check.  
Download polenum from http://labs.portcullis.co.uk/application/polenum/\n\n";
		return 0;
	}
	...
	$command = "rpcclient -W '$global_workgroup' -U'$global_username'\%'$global_password' 
'$global_target' -c \"getdompwinfo\" 2>&1";
	print "[V] Attempting to get Password Policy info with command: $command\n" if $verbose;
...
sub enum_groups {
	...
	foreach my $grouptype ("builtin", "domain") {
		# Get list of groups
		my $command = "rpcclient -W '$global_workgroup' 
-U'$global_username'\%'$global_password' '$global_target' -c 'enumalsgroups $grouptype' 2>&1";
		...
		foreach my $groupname (keys %rid_of_group) {
			...
			$command = "net rpc group members '$groupname' -W '$global_workgroup' -I 
'$global_target' -U'$global_username'\%'$global_password' 2>&1\n";
			print "[V] Running command: $command\n" if $verbose;
			my $members = `$command`;
			my @members = split "\n", $members;
			foreach my $m (@members) {
				print "Group '$groupname' (RID: " . $rid_of_group{$groupname} . ") 
has member: $m\n";
			}
		}
		if ($global_detailed) {
			foreach my $groupname (keys %rid_of_group) {
				print "[+] Getting detailed info for group $groupname (RID: " . 
$rid_of_group{$groupname} . ")\n";
...
sub enum_dom_groups {
	# Get list of groups
	my $command = "rpcclient -W '$global_workgroup' -U'$global_username'\%'$global_password' 
'$global_target' -c \"enumdomgroups\" 2>&1";
...
'$global_target' -U'$global_username'\%'$global_password' 2>&1\n";
		...
		my @members = split "\n", $members;
		foreach my $m (@members) {
			print "Group '$groupname' (RID: " . $rid_of_group{$groupname} . ") has 
member: $m\n";
		}
	}
	if ($global_detailed) {
...
sub enum_shares {
	...
	# my $shares = `net rpc share -W '$global_workgroup' -I '$global_target' 
-U'$global_username'\%'$global_password' 2>&1`;
	my $command = "smbclient -W '$global_workgroup' -L //'$global_target' 
-U'$global_username'\%'$global_password' 2>&1";
	my $shares = `$command`;
	if (defined($shares)) {
	...
	foreach my $share (@shares) {
		$share =~ s/'/'\\''/g;
		my $command = "smbclient -W '$global_workgroup' //'$global_target'/'$share' 
-U'$global_username'\%'$global_password' -c dir 2>&1";
...
sub enum_shares_unauth {
	...
	foreach my $share (@shares) {
		...
		my $result = `smbclient -W '$global_workgroup' //'$global_target'/'$share' -c dir 
-U'$global_username'\%'$global_password' 2>&1`;
		if ($result =~ /blocks of size .* blocks available/) {
			print "$share EXISTS, Allows access using username: '$global_username', 
password: '$global_password'\n";
...
sub enum_users_rids {
	...
	foreach my $known_username (@global_known_usernames) {
		my $command = "rpcclient -W '$global_workgroup' 
-U'$global_username'\%'$global_password' '$global_target' -c 'lookupnames $known_username' 2>&1";
...
$verbose;
		...
		if ($sid =~ /NT_STATUS_ACCESS_DENIED/) {
			print "[E] Couldn't get SID: NT_STATUS_ACCESS_DENIED.  RID cycling not 
possible.\n";
			last;
		} elsif ($sid =~ /NT_STATUS_NONE_MAPPED/) {
			print "[V] User \"$known_username\" doesn't exist.  User enumeration should 
be possible, but SID needed...\n" if $verbose;
			next;
	...
	my $command = "rpcclient -W '$global_workgroup' -U'$global_username'\%'$global_password' 
'$global_target' -c lsaenumsid 2>&1";
...
possible.\n";
	...
	foreach my $sid (keys %sids) {
		if (! defined($sid) and $global_username) {
			print "[V] WARNING: Can\'t get SID.  Maybe none of the 'known' users really 
exist.  Try others with -k.  Trying null session.\n" if $verbose;
			foreach my $known_username (@global_known_usernames) {
				my $command = "rpcclient -W '$global_workgroup' -U% '$global_target' 
-c 'lookupnames $known_username' 2>&1";
...
$verbose;
			...
			unless (defined($sid)) {
				print "[E] Can't get SID using either a null username or the username 
\"$global_username\"\n";
				exit 1;
			}
		...
		foreach my $rid_range (@ranges) {
			...
			# Invalid range
				...
				print "WARNING: RID range $rid_range isn't valid.  Should be like 
10-20 or 1199.  Ignoring this range\n";
				next;
			}
		
			# Check we have an ascending range
			if ($start_rid > $end_rid) {	
				print "WARNING: RID range $rid_range seems to be reversed.  
Automatically reversing.\n";
			...
			foreach my $rid ($start_rid..$end_rid) {
				my $output = `rpcclient -W '$global_workgroup' 
-U'$global_username'\%'$global_password' '$global_target' -c 'lookupsids $sid-$rid' 2>&1`;
				...
				if ($sid_and_user) {
					...
					if ($sid_and_user =~ /-(\d+) .*\\\1 \(/) {
						...
						print "$sid_and_user\n";
						$fail_count = 0;
						get_user_details_from_rid($rid) if $sid_and_user =~ 
/\((Local|Domain) User\)/;
						get_group_details_from_rid($rid) if $sid_and_user =~ 
/\((Local|Domain) Group\)/;
					}
				} else {
					$fail_count++;
...
sub enum_users {
	...
	my $command = "rpcclient -W '$global_workgroup' -c querydispinfo 
-U'$global_username'\%'$global_password' '$global_target' 2>&1";
	...
	print "\n";
	$command = "rpcclient -W '$global_workgroup' -c enumdomusers 
-U'$global_username'\%'$global_password' '$global_target' 2>&1";
	print "[V] Attempting to get userlist with command: $command\n" if $verbose;
	$users = `$command`;
	if ($users =~ /NT_STATUS_ACCESS_DENIED/) {
...
sub get_group_details_from_rid {
	...
	my $command = "rpcclient -W '$global_workgroup' -U'$global_username'\%'$global_password' -c 
'querygroup $rid' '$global_target' 2>&1";
...
sub get_user_details_from_rid {
	...
	return unless $global_detailed;
	my $command = "rpcclient -W '$global_workgroup' -U'$global_username'\%'$global_password' -c 
'queryuser $rid' '$global_target' 2>&1";
...
sub get_printer_info {
	...
	my $command = "rpcclient -W '$global_workgroup' -U'$global_username'\%'$global_password' -c 
'enumprinters' '$global_target' 2>&1";
	...
	foreach my $line (@nbt_in) {
		if ($line =~ /\s+(\S+)\s+<(..)>\s+-\s+?(<GROUP>)?\s+?[A-Z]/) {
			...
			foreach my $info_aref (@nbt_info) {
				...
				# print "Matching: line=\"$line\", val=$line_val, code=$line_code, 
group=$line_group against pattern=$pattern, code=$code, group=$group, desc=$desc\n";
				if ($pattern) {
					if ($line_val =~ /$pattern/ and $line_code eq $code and 
$line_group eq $group) {
