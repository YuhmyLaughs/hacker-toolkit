...
def load_file(filename, insensitive):
    """
    Load each line of a file into a list and then convert the list to a 
set to
...
def build_filenames(name_list, ext_list):
    ...
    if len(ext_list) == 0:
        ...
        names = set(['{0}{1}'.format(n, e) for n in name_list for e in 
ext_list])
...
def head(url):
    """
    Make a HEAD request to the URL. If we do not get a 404 then the URL 
is
    ...
    if resp.status_code in [301, 302]:
        print('[+] Found {0} --> {1} ({2})'.format(url, 
resp.headers['location'], resp.status_code))
        return url
...
def check(base_url, path_list, dir=True):
    ...
    # If we have a list of directories force them to end with a forward 
slash.
    if dir is True:
...
parser.add_argument('directory_file', action='store',
                    help="List of directories to enumerate. One per 
line.")
parser.add_argument('-n', action='store', default=None, 
metavar='name_file',
                    help="List of filenames to enumerate. One per 
line.")
parser.add_argument('-e', action='store', default=None, 
metavar='extension_file',
                    help='List of extensions to append to filenames. One 
per line.')
parser.add_argument('-i', action='store_true', default=False,
                    help='Use case insensitive directory and 
filenames.')
parser.add_argument('-o', action='store', default='resources.txt', 
metavar='output_file',
